function[Xtrain,Xtest,Ytrain,Ytest,vSelectedSample,vNotSelectedSample]=spxy(X,y,Ncal)
...
[mx,~]=size(X);
vAllofSample=1:mx;
vAllofSample(:,m)=[];n=vAllofSample;
Xtrain=X(m,:);Ytrain=y(m,:);Xcopy(m,:)=[];Ycopy(m,:)=[];
Xtest=Xcopy;Ytest=Ycopy;
vSelectedSample=m;vNotSelectedSample=n;
end